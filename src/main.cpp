...
int main(void)
{
    ...
    //load mesh
    res::Model bunny_mesh;
    res::load_model("binary/drill.bin", &bunny_mesh);

    //load model and add object to scene
    unsigned int bunny_id = res::order_model("binary/drill.bin", loader, res_cache, gfx_cache);
    ...
    normalize_transform({0, 0, 0}, 1, (float*)bunny_mesh.aabb_min, (float*)bunny_mesh.aabb_max, obj->m_trans);
    obj->update_mat();
    ...
    Point points[n];
    part_seg.m_points = (Point*)points;
    
    for (int i = 0; i < n; i++) 
    {
        sample_mesh(&bunny_mesh, (float*)s_pt, (float*)s_norm);
        glm::vec4 pt = {s_pt[0], s_pt[1], s_pt[2], 1};
        pt = obj->trans_mat * pt;
        gfx::ShapeEntry* ent = debug_renderer.shape(gfx::Shape::CUBE);
        game::transform_mat({{pt[0], pt[1], pt[2]}, {1, 1, 1}, 0, {.008, .008, .008}}, ent->mat);
        game::set_color(s_norm[0], s_norm[1], s_norm[2], 1, ent->color);
        points[i] = {{pt[0], pt[1], pt[2]}, {s_norm[0], s_norm[1], s_norm[2]}, 0, ent, false};
    }
